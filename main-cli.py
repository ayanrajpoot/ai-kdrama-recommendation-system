...
# First, I preprocess the data by filling in any missing values in the 'genres', 'tags', and 'content' columns
# and cleaning up the tags. Then, I search for the target show and get its index. I use TfidfVectorizer to
# vectorize the tags, content, and genres of all the shows. For measuring similarity, I apply K-Nearest
# Neighbors (KNN) to the tags and cosine similarity to the content and genres. I combine these similarity s
# cores using weights for each category—tags, content, and genres—resulting in a total similarity index.
# Finally, I return the top 10 most similar shows, sorted by their similarity index, along with their names,
# genres, tags, and similarity scores.
...
def find_similar_shows( df, target_show_name, knn_neighbors=50, tag_weight=0.60, content_weight=0.10, genre_weight=0.30):
    ...
    return top_10[['name', 'genres', 'tags', 'total_similarity_index']]
